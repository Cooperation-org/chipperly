...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`_pages_v_version_faqs\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`question\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_pages_v_version_faqs_order_idx\` ON \`_pages_v_version_faqs\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_version_faqs_parent_id_idx\` ON \`_pages_v_version_faqs\` (\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`waitlist_email_idx\` ON \`waitlist\` (\`email\`);`)
  ...
  await db.run(sql`CREATE TABLE \`exports\` (
  	...
  	\`collection_slug\` text DEFAULT 'waitlist' NOT NULL,
  	\`where\` text DEFAULT '{}',
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`url\` text,
  ...
  await db.run(sql`CREATE TABLE \`settings_share\` (
  	\`order\` integer NOT NULL,
  	\`parent_id\` integer NOT NULL,
  	\`value\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`settings_share_order_idx\` ON \`settings_share\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`settings_share_parent_idx\` ON \`settings_share\` (\`parent_id\`);`)
  await db.run(sql`CREATE TABLE \`settings\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`announcement_enabled\` integer,
  	\`announcement_text\` text,
  	\`announcement_link\` text,
  	\`launched\` integer,
  	\`contact_email\` text DEFAULT 'info@chipperlyapp.com',
  	\`clarity_id\` text,
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`_pages_v_version_faqs\`;`)
  ...
  await db.run(sql`DROP TABLE \`settings_share\`;`)
