 ...
 * turns app blocking on; a plain lock leaves it as it is. Every queued write
 * is pushed first, before /me/lock locks the session: the sync lock gate
 * refuses profile writes after that, and a settings change saved just
 * before locking was rejected and lost. The server sets child_mode_active
 * too, via block_apps.
 */
