...
Sync authorization: user may read/write profile P if P.account_id is an account where user is `admin`, or user is `member` and (P.id in profile_members for user). A device is child-locked when this session's `sessions.locked_profile_id` is set (POST `/me/lock`; cleared by POST `/me/unlock`, which requires the caregiver's PIN, checked server-side against `users.pin_hash`). While locked, sync/push restricts writes to `schedule_items` (only `completed_at`, `completed_by`, `client_updated_at` may change, plus `position` when `settings.child_reorders === true`), `step_completions`, `attitude_checks`, `mood_events`, `chip_ledger` with reason `task|step|routine`, plus two child-view actions the caregiver can switch off in the profile's settings: `chip_ledger` with reason `redeem` (unless `settings.child_redeems === false`) and `locations` where only `working_for_reward_id` differs from the stored row (unless `settings.child_picks_reward === false`).
...
lib/data/schedule.ts     useDayItems(profileId, isoDate) -> DayItem[] with activity + steps (flat, pre-order walk of the step tree, each with `depth`) + completions joined ; stepTree(steps) -> StepNode[] ; descendantsOf(steps, stepId) -> string[] ; addToDay ; setCompleted(itemId, done, by) ; setStepCompleted(itemId, stepId, done, by) (cascades: checking/unchecking a step propagates to every descendant and recomputes every ancestor up the chain) ; setItemStory(itemId, storyId | null) (schedule_items.story_id, S7's Story row and S32's Read story button) ; removeFromDay(itemId, scope: 'today'|'always') ; reorder ; materializeRecurring(profileId, isoDate) ; copyDay(from, to)
lib/data/activities.ts   useActivities(profileId) ; useActivity(id) ; saveActivity(input) (steps are a flat array, each with optional parent_step_id + position among siblings; reconciles upsert/soft-delete, cascading soft-delete to a removed step's descendants) ; deleteActivity(id) ; recent
